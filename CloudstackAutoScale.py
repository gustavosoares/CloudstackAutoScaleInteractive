...
def listTemplates(templatefilter):
    ...
    for template in templates:
        table.add_row(
            [template['id'], template['name'], template['displaytext'], template['isready'], template['isfeatured']])
...
def listAutoScalePolicies(projectid):
    ...
    for autoscalepolicy in autoscalepolicies['autoscalepolicy']:
        ...
        for condition in conditions:
            counters = (condition.get('counter'))
            for counter in counters:
                table.add_row([autoscalepolicy['id'], autoscalepolicy['action'], autoscalepolicy['duration'],
                               autoscalepolicy['quiettime'], counter['name'], condition['relationaloperator'],
                               condition['threshold']])
...
def listAutoScaleVmGroup(projectid):
    ...
    table = PrettyTable(
        ["ID", "LoadBalancer", "Interval", "Max", "Min", "Running", "Action", "Counter", "Operator", "% Threshold"])
        ...
        for scaledownpolicy in scaledownpolicies:
            ...
            for conditiond in conditionsd:
                ...
                for counterd in countersd:
                    table.add_row(
                        [vmgroup['id'], lbname, vmgroup['interval'], vmgroup['maxmembers'], vmgroup['minmembers'],
                         numvmsrunning, scaledownpolicy['action'], counterd['name'], conditiond['relationaloperator'],
                         conditiond['threshold']])
        for scaleuppolicy in scaleuppolicies:
            ...
            for conditionu in conditionsu:
                ...
                for counteru in countersu:
                    table.add_row(
                        [vmgroup['id'], lbname, vmgroup['interval'], vmgroup['maxmembers'], vmgroup['minmembers'],
                         numvmsrunning, scaleuppolicy['action'], counteru['name'], conditionu['relationaloperator'],
                         conditionu['threshold']])
...
cloudstack = cloudstack_autoscale.Client(api, apikey, secret)
    ...
    if args.command == 'counter':
        ...
        try:
            duration = int(raw_input(
                Colors.BOLD + "Enter the duration (seconds) for which the condition have to be true before action is taken: " + Colors.ENDC))
        except ValueError:
            print Colors.FAIL + "Error: this value is not integer" + Colors.ENDC
            sys.exit(1)
